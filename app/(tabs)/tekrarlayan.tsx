...
export default function TekrarlayanScreen() {
  ...
  const totalMonthly = tekrarlayanOdemeler
    .filter(o => o.frequency === 'aylik')
    .reduce((sum, o) => sum + o.amount, 0);

  const overdueCount = tekrarlayanOdemeler.filter(o => isOverdue(o.next_date)).length;
      ...
      <TouchableOpacity style={[
        ...
        <View style={styles.odemeLeft}>
          ...
          <View style={styles.odemeInfo}>
            <Text style={styles.odemeName}>{item.name}</Text>
            <View style={styles.odemeMeta}>
              <Calendar size={12} color="#6b7280" />
              <Text style={[
                styles.odemeDate,
                overdue && styles.odemeDateOverdue
              ]}>
                {overdue ? 'Gecikti: ' : ''}{formatDate(item.next_date)}
              </Text>
              <Text style={styles.odemeFrequency}>• {frequencyLabels[item.frequency]}</Text>
            </View>
            {item.cari && (
        ...
        <Text style={[
          styles.odemeAmount,
          overdue && styles.odemeAmountOverdue
        ]}>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      {/* Add Modal */}
      <AddTekrarlayanOdemeModal visible={showAddModal} onClose={() => setShowAddModal(false)} />
    </SafeAreaView>
  );
}
