...
export default function Islemler() {
  const { islemler, loadingIslemler, fetchIslemler, fetchProfile, profile, fetchKasalar, fetchCariler, fetchKategoriler } = useStore();
  ...
  const filteredIslemler = islemler.filter(islem => {
    ...
    if (filter === 'gelir') return islem.type === 'gelir' || islem.type === 'tahsilat';
    if (filter === 'gider') return islem.type === 'gider' || islem.type === 'odeme';
    return true;
  });
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity style={styles.addButton} onPress={() => openAddModal('gider')}>
      ...
      >
        ...
        <TouchableOpacity
          style={[styles.filterChip, filter === 'gelir' && styles.filterChipActiveGreen]}
          onPress={() => setFilter('gelir')}
        >
          <ArrowDownLeft size={14} color={filter === 'gelir' ? '#fff' : '#10b981'} />
          <Text style={[styles.filterChipText, filter === 'gelir' && styles.filterChipTextActive]}>
            Gelirler
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[styles.filterChip, filter === 'gider' && styles.filterChipActiveRed]}
          onPress={() => setFilter('gider')}
        >
          <ArrowUpRight size={14} color={filter === 'gider' ? '#fff' : '#ef4444'} />
          <Text style={[styles.filterChipText, filter === 'gider' && styles.filterChipTextActive]}>
            Giderler
          </Text>
        </TouchableOpacity>
      </ScrollView>
      ...
      <View style={styles.summaryRow}>
        <View style={styles.summaryItem}>
          ...
          <Text style={styles.summaryAmountPositive}>{formatCurrency(totalGelir)}</Text>
        </View>
        <View style={styles.summaryDivider} />
        <View style={styles.summaryItem}>
          <ArrowUpRight size={16} color="#ef4444" />
          <Text style={styles.summaryLabel}>Gider</Text>
          <Text style={styles.summaryAmountNegative}>{formatCurrency(totalGider)}</Text>
        </View>
        <View style={styles.summaryDivider} />
        <View style={styles.summaryItem}>
      ...
      {filteredIslemler.length > 0 ? (
        ...
        <View style={styles.emptyContainer}>
          ...
          <View style={styles.actionButtons}>
            <TouchableOpacity style={styles.incomeButton} onPress={() => openAddModal('gelir')}>
              <ArrowDownLeft size={20} color="#fff" />
              <Text style={styles.actionButtonText}>Gelir Ekle</Text>
            </TouchableOpacity>
            <TouchableOpacity style={styles.expenseButton} onPress={() => openAddModal('gider')}>
